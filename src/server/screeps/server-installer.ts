...
export function screepsrcContent(): string {
  return [
    '; screeps-arena managed private server (generated)',
    '; 端口在启动时通过 CLI 参数注入，这里只写静态默认值。',
    'steam_api_key = screeps-arena-placeholder',
    'host = 127.0.0.1',
    'cli_host = 127.0.0.1',
    'password =',
    'runners_cnt = 1',
...
export function serverPackageJson(screepsVersion: string, simplebotVersion: string | false): string {
  ...
  return `${JSON.stringify({ name: 'screeps-arena-server', version: '0.0.0', private: true, dependencies: deps }, null, 2)}\n`
}
...
export async function ensureScreepsServer(options: ServerInstallOptions): Promise<ServerInstallResult> {
  ...
  const markerPath = path.join(nodeModules, '.screeps-arena-server.json')
  ...
  const secretPath = path.join(serverDir, '.screeps-arena-secret')
  if (!existsSync(secretPath)) {
    await writeFile(secretPath, randomBytes(24).toString('hex'))
    log('generated .screeps-arena-secret')
  }
...
export async function readArenaSecret(serverDir: string): Promise<string | undefined> {
  try {
    const content = await readFile(path.join(serverDir, '.screeps-arena-secret'), 'utf8')
 ...
 *   3. 写 marker `<driverDir>/.screeps-arena-driver-patched`：含 patched hash + 时间戳。
 *
 * 幂等：sentinel `// screeps-arena patch: accessibleRooms-fallback` 写在 bundle 末尾 +
...
export async function patchDriverBundle(
  ...
  const markerPath = path.join(driverDir, '.screeps-arena-driver-patched')
  const sentinel = '// screeps-arena patch: accessibleRooms-fallback'
...
async function patchAccessibleRoomsInflight(driverDir: string, log: (line: string) => void): Promise<void> {
  ...
  const marker = '// screeps-arena patch: accessibleRooms-inflight'
  if (!existsSync(file)) {
