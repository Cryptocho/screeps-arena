'use strict'
/**
 * dsh-screeps arena mod — 跑在 Screeps 私服 backend 进程内的控制面。
 *
 ...
 * 2. /api/arena/* 控制面：建用户(带 spawn 部署)/发 token/世界快照/系统控制。
 *    host 通过这些路由管理对局；浏览器与 Agent 永远拿不到裸 token。
 *
 * 安全边界：
 * - 仅接受 loopback 来源；
 * - 可选共享密钥：serverDir/.dsh-arena-secret 文件存在时强制校验 X-Arena-Secret 头；
...
;(function () {
  ...
  function stubSteamWebApi() {
    if (!process.env.STEAM_KEY) {
      ...
      process.env.STEAM_KEY = 'dsh-screeps-placeholder'
    }
    try {
  ...
  function readSecret() {
    try {
      ...
      var file = path.join(process.cwd(), '.dsh-arena-secret')
  ...
  function subscribeConsole(userId) {
    ...
    try {
      ...
      console.error('[dsh-screeps] console subscribe failed for ' + userId + ':', e)
    }
  }
  ...
  /* ------------------------------------------------------------------ */
  /* Canonical replay bridge（M4-A0 正式 bridge，schemaVersion=1）        */
  /*                                                                    */
  /* 产品语义（plan-M4 v6 §5.2/§5.3）：arena-mod 在 roomsDone 边界把     */
  /*「accepted tick」产出为 frame（每 tick 一条）或 gap record（闭区间）   */
  /*   [fromTick,toTick]，按 record seq 单调递增，绝不静默丢 tick；      */
  /* host 经 /api/arena/system replayStart/Page/Stop 消费。             */
  /*                                                                    */
  /* 本模块是纯状态机 + 队列，不直接碰 db/env；帧内容由注入的            */
  /* makeFrame(gameTime) 提供（生产路径=arena-mod 白名单快照，契约测试=  */
  /* fake 帧），这样契约测试可完全独立驱动。                              */
  /*                                                                    */
  /* bridge 内部语义：                                                  */
  /* - replayStart：建立新 generation；只允许一个 active bridge；         */
  /* - acceptTick(gameTime)：bridge active 且未 closed 时把 tick 记为    */
  /*   accepted：能生成 frame 就入 frame 队列，不能则并入 gap 区间；      */
  /* - 队列深度超限且当前无 gap 可并入 → 标记 fatalBackpressure 并拒绝   */
  /*   新 tick（显式可观察，不静默丢弃）；                                */
  /* - replayStop：closed → drain → finalize status/complete。          */
  /* ------------------------------------------------------------------ */
  ...
  function createReplayBridge(opts) {
    opts = opts || {}
    var capacity = opts.queueCapacity || 256
    var makeFrame = opts.makeFrame || function (gameTime) { return Promise.resolve({ gameTime: gameTime, rooms: [], events: [] }) }
    var state = {
      schemaVersion: 1,
      active: false,
      replayId: null,
      ...
      lastPublishedTick: null,
      records: [], // [{kind:'frame'|'gap', seq, ...}]，按 seq 递增
      queue: [], // 尚未产出 frame 的 accepted gameTime（FIFO）
      gap: null, // {fromTick,toTick,reason} 待发布 gap（闭区间）
      closed: false,
      status: 'live', // live | complete | partial
      fatalBackpressure: false,
      gapReasons: [],
      queueCapacity: capacity,
      drainChain: Promise.resolve(), // 串行链保证 frame 产出保序
    }
    ...
    function reset() {
      ...
      state.matchId = null
      state.rooms = null
      state.participantSnapshot = null
      state.lastAcceptedTick = null
      state.lastPublishedTick = null
      state.records = []
      state.queue = []
      state.gap = null
      state.closed = false
      state.status = 'live'
      state.fatalBackpressure = false
      state.gapReasons = []
      state.drainChain = Promise.resolve()
    }

    function start(input) {
      if (state.active) {
        var conflict = new Error('replay already active: ' + state.replayId)
        conflict.code = 'conflict'
        return { error: conflict }
      }
      // 全新 generation：清空旧 generation 的任何缓冲（records/queue/gap/diagnostics）
      state.active = true
      state.replayId = String(input.replayId)
      state.sourceGeneration = 'g-' + Math.random().toString(36).slice(2, 10) + Date.now().toString(36)
      state.matchId = input.matchId ? String(input.matchId) : null
      state.rooms = Array.isArray(input.rooms) ? input.rooms.slice() : []
      state.participantSnapshot = input.participantSnapshot || null
      state.lastAcceptedTick = typeof input.acceptedGameTime === 'number' ? input.acceptedGameTime : null
      state.records = []
      state.queue = []
      state.gap = null
      state.closed = false
      ...
      return {
        ...
        cursor: 0,
        acceptedGameTime: state.lastAcceptedTick,
        queueCapacity: capacity,
      }
    }

    function publishGap(fromTick, toTick, reason) {
      state.records.push({
        schemaVersion: 1,
        sourceGeneration: state.sourceGeneration,
      ...
      if (state.gapReasons.indexOf(reason) === -1) state.gapReasons.push(reason)
      state.gap = null
      state.lastPublishedTick = toTick
    }

    function publishFrame(tick, frame) {
      state.records.push({
        schemaVersion: 1,
        sourceGeneration: state.sourceGeneration,
        replayId: state.replayId,
      ...
      // frame 后若有待发布 gap，先发布（保持严格 tick 顺序：gap 必然在更早 tick 上）
      if (state.gap && state.gap.toTick < tick) {
        publishGap(state.gap.fromTick, state.gap.toTick, state.gap.reason)
      }
    }

    /** 串行 drain：按 FIFO 逐个 async 产出 frame；失败并入 gap。 */
    function drain() {
      state.drainChain = state.drainChain.then(function () {
        return (function step() {
          if (state.queue.length === 0 || !state.active) return Promise.resolve()
          if (state.closed && false) return Promise.resolve() // closed 仍允许 drain 完
          var tick = state.queue[0]
          return Promise.resolve()
            .then(function () { return makeFrame(tick, state) })
            .then(function (frame) {
              state.queue.shift()
              publishFrame(tick, frame)
              return step()
            })
            .catch(function (e) {
              state.queue.shift()
              // 帧失败：并入 busy gap；若已存在 gap 则扩展闭区间
              if (state.gap && state.gap.reason === 'busy' && state.gap.toTick >= tick - 1) {
                state.gap.toTick = Math.max(state.gap.toTick, tick)
              } else {
                if (state.gap) publishGap(state.gap.fromTick, state.gap.toTick, state.gap.reason)
                state.gap = { fromTick: tick, toTick: tick, reason: 'busy' }
              }
              return step()
            })
        })()
      })
      return state.drainChain
    }

    /**
     * roomsDone 每 tick 调用（async）。active 且未 closed 时：
     *  - 同 tick 重复/倒退 → 不 accepted；
     *  - 队列有空 → 入队并 drain；
     *  - 队列满 → 并入/扩展 gap（backpressure），仍 accepted（被 gap 覆盖）。
     * 队列满且无 gap 挂起时直接 fatalBackpressure（显式可观察，绝不静默丢）。
     */
    function acceptTick(gameTime) {
      var tick = Math.max(0, Number(gameTime) || 0)
      if (!state.active) return Promise.resolve({ accepted: false, reason: 'inactive' })
      if (state.closed) return Promise.resolve({ accepted: false, reason: 'closed' })
      if (tick === state.lastAcceptedTick) return Promise.resolve({ accepted: false, reason: 'duplicate' })
      if (state.lastAcceptedTick != null && tick < state.lastAcceptedTick) {
        return Promise.resolve({ accepted: false, reason: 'stale' })
      }
      if (state.fatalBackpressure) return Promise.resolve({ accepted: false, reason: 'fatalBackpressure' })
      state.lastAcceptedTick = tick
      // backlog 未超限且无 gap 挂起 → 入队异步产 frame；否则并入/扩展 gap（显式 backpressure）
      if (state.queue.length < capacity && state.gap == null) {
        state.queue.push(tick)
        return drain().then(function () {
          checkFatal()
          // 若 drain 已把该 tick 折进 gap（帧产出失败），返回 gap 语义
          return { accepted: true, kind: state.gap && state.gap.toTick === tick ? 'gap' : 'frame', cursor: state.records.length }
        })
      }
      // backlog 已满或已有 gap 挂起：并入/扩展 gap，闭区间连续
      if (state.gap) {
        if (tick > state.gap.toTick + 1) {
          publishGap(state.gap.fromTick, state.gap.toTick, state.gap.reason)
          state.gap = { fromTick: tick, toTick: tick, reason: 'backpressure' }
        } else {
          state.gap.toTick = tick
        }
      } else {
        state.gap = { fromTick: tick, toTick: tick, reason: 'backpressure' }
      }
      checkFatal()
      return Promise.resolve({ accepted: true, kind: 'gap', cursor: state.records.length })
    }

    /** 持续无法产出/积压无限增长 → 显式 fatal（绝不静默丢，stop 报告 partial）。 */
    function checkFatal() {
      if (state.fatalBackpressure) return
      var backlog = state.queue.length
      var gapSpan = state.gap ? state.gap.toTick - state.gap.fromTick + 1 : 0
      // 阈值：gap 跨度超过 max(16, capacity*4) 或积压超过 capacity*4 → producer 持续失败视为 fatal
      if (backlog >= capacity * 4 || gapSpan >= Math.max(16, capacity * 4)) {
        state.fatalBackpressure = true
        if (state.gapReasons.indexOf('fatalBackpressure') === -1) state.gapReasons.push('fatalBackpressure')
      }
    }

    function flushGapAndDrain() {
      if (state.closed && state.gap) {
        publishGap(state.gap.fromTick, state.gap.toTick, state.gap.reason)
      }
    }

    function page(cursor, limit) {
      if (!state.active) return { error: { code: 'inactive', message: 'no active replay' } }
      var from = typeof cursor === 'number' && cursor >= 0 ? Math.floor(cursor) : 0
      var lim = typeof limit === 'number' && limit > 0 ? Math.min(Math.floor(limit), 200) : 100
      var records = state.records.filter(function (r) { return r.seq >= from }).slice(0, lim)
      var nextCursor = records.length > 0 ? records[records.length - 1].seq + 1 : from
      var partial = state.closed && (state.fatalBackpressure || state.gapReasons.length > 0 || state.gap != null || state.queue.length > 0)
      return {
        schemaVersion: 1,
        sourceGeneration: state.sourceGeneration,
        replayId: state.replayId,
        ...
        records: records,
        status: state.closed ? (partial ? 'partial' : 'complete') : 'live',
        complete: state.closed && !partial,
        finalCursor: state.closed ? state.records.length : undefined,
        gapReasons: state.gapReasons.slice(),
        fatalBackpressure: state.fatalBackpressure,
      }
    }
    ...
    function stop() {
      ...
      if (!state.closed) {
        ...
        return drain().then(function () {
          if (state.gap) publishGap(state.gap.fromTick, state.gap.toTick, state.gap.reason)
          var partial = state.fatalBackpressure || state.gapReasons.length > 0 || state.queue.length > 0
          var final = {
            schemaVersion: 1,
            sourceGeneration: state.sourceGeneration,
            replayId: state.replayId,
            finalCursor: state.records.length,
            finalGameTime: state.lastAcceptedTick,
          ...
          state.records = state.records.slice() // final manifest 持有引用快照
          return final
        })
      }
      // 已 closed：幂等返回同一 final
      var partial2 = state.fatalBackpressure || state.gapReasons.length > 0 || state.queue.length > 0
      return Promise.resolve({
        schemaVersion: 1,
        sourceGeneration: state.sourceGeneration,
        replayId: state.replayId,
    ...
    function status() {
      var partial = state.closed && (state.fatalBackpressure || state.gapReasons.length > 0 || state.queue.length > 0)
      return {
        schemaVersion: 1,
        active: state.active,
        replayId: state.replayId,
        ...
        status: !state.active ? 'inactive' : state.closed ? (partial ? 'partial' : 'complete') : 'live',
        complete: state.closed && !partial,
        cursor: state.records.length,
    ...
    /** reset/restart：finalize 当前 bridge 为 partial 并释放 active，旧 generation 不可再写。 */
    function invalidate(reason) {
      if (state.active) {
        state.closed = true
        return drain().then(function () {
          if (state.gap) publishGap(state.gap.fromTick, state.gap.toTick, state.gap.reason)
          state.gapReasons.push(reason)
          var result = {
            invalidated: true,
          ...
          // 释放 active：同 bridge 可开新 generation；旧 records 保留在 state 供审计，
          // 但下次 start 会清空（新 generation 与旧 cursor/generation 天然隔离）
          state.active = false
          return result
        })
      }
  ...
  /**
   * 生产帧工厂：以 db/env 的白名单公开字段构造每 tick canonical frame。
   * 不接触 token/源码/Memory/私有 console/内部 _id/store/intents。
   * bridge 状态作为第二参传入，提供 rooms 与 participantSnapshot。
   * rooms 元数据（status/novice/...）与房间对象每次独立查询（帧即快照）。
   */
  function defaultFrameProducer(gameTime, bridgeState) {
    var common = deps().common
    var db = common.storage.db
    var rooms = bridgeState && bridgeState.rooms && bridgeState.rooms.length > 0 ? bridgeState.rooms : null
    if (!rooms || rooms.length === 0) {
      return Promise.resolve({ schemaVersion: 1, gameTime: gameTime, rooms: [], events: [] })
    }
    var roomMeta = {}
    var usernameById = {}
    var eventSummary = []
    var roomObjects = [] // 后续 then 使用
    return common
      .getGametime()
      .catch(function () { return gameTime })
      .then(function (gt) {
        void gt
        // 房间元数据：status/novice/respawnArea/openTime
        return db.rooms.find({ _id: { $in: rooms } })
      })
      .then(function (roomRows) {
        roomRows.forEach(function (r) {
          roomMeta[r._id] = { status: r.status, novice: r.novice, respawnArea: r.respawnArea, openTime: r.openTime }
        })
        // 房间内公开对象：只取白名单类型与字段
        return db['rooms.objects'].find({ room: { $in: rooms }, type: { $in: ['controller', 'spawn', 'tower', 'creep', 'constructionSite'] } })
      })
      .then(function (objects) {
        roomObjects = objects || []
        // 收集 owner user id 一次性解析 username（公开昵称）
        var ownerIds = []
        var seen = {}
        roomObjects.forEach(function (o) {
          if (o.user && !seen[o.user]) { seen[o.user] = true; ownerIds.push(o.user) }
        })
        if (ownerIds.length === 0) return Promise.resolve({})
        return db.users.find({ _id: { $in: ownerIds } }, { _id: true, username: true }).then(function (users) {
          var map = {}
          users.forEach(function (u) { map[u._id] = u.username })
          return map
        })
      })
      .then(function (usernameByIdResult) {
        usernameById = usernameByIdResult || {}
        var byRoom = {}
        rooms.forEach(function (room) { byRoom[room] = { room: room, status: (roomMeta[room] || {}).status, publicObjects: [] } })
        roomObjects.forEach(function (o) {
          if (!byRoom[o.room]) return
          var username = usernameById[o.user] || null
          var obj = { kind: o.type, x: o.x, y: o.y }
          if (o.type === 'controller') {
            obj.level = o.level || 0
            obj.username = username
          } else {
            obj.username = username
            if (typeof o.hits === 'number') obj.hitsBucket = Math.min(15, Math.floor(o.hits / 1000))
          }
          byRoom[o.room].publicObjects.push(obj)
        })
        var roomsOut = rooms.map(function (room) {
          var meta = roomMeta[room] || {}
          var out = {
            room: room,
            status: meta.status,
            novice: !!meta.novice,
            respawnArea: !!meta.respawnArea,
            openTime: meta.openTime || 0,
          }
          var row = byRoom[room] || { publicObjects: [] }
          out.publicObjects = row.publicObjects
          // controller 归属折叠进 own（map-stats-equivalent）
          var controller = row.publicObjects.find(function (o) { return o.kind === 'controller' })
          if (controller && controller.username) {
            out.own = { username: controller.username, level: controller.level || 0 }
          } else {
            out.own = null
          }
          return out
        ...
        return {
          ...
          gameTime: gameTime,
          rooms: roomsOut,
          events: eventSummary, // M4-A0 帧事件摘要留空/由 D 步事件映射接入
        }
      })
  }

  // arena-mod 生产持有的唯一 replay bridge（惰性创建；frame 内容来自 db 白名单快照）
  var replayBridge = null
  function getReplayBridge() {
    if (!replayBridge) replayBridge = createReplayBridge({ queueCapacity: 256, makeFrame: defaultFrameProducer })
    return replayBridge
  }
  function resetReplayBridge() {
    if (replayBridge) replayBridge.reset()
    replayBridge = null
  }
  ...
  function ensureEventCollector() {
    ...
    try {
      ...
      deps().common.storage.pubsub.subscribe('roomsDone', function (gameTime) {
        pubsubProbe.ticks++
        // M4-A0：canonical replay bridge 每 tick accept（frame 或 gap，不静默丢）
        if (replayBridge && replayBridge.state.active) {
          replayBridge.acceptTick(Math.max(0, Number(gameTime) || 0)).catch(function (e) {
            console.error('[dsh-screeps] replay accept failed:', e && e.stack ? e.stack : e)
          })
        }
        ...
        collectEventLog(Math.max(0, Number(gameTime) || 0)).then(
          ...
          function (e) {
            eventBusy = false
            console.error('[dsh-screeps] event collect failed:', e && e.stack ? e.stack : e)
          },
        )
      })
    } catch (e) {
      console.error('[dsh-screeps] event collector subscribe failed:', e)
  ...
  function ensureRoomStatusOnce() {
    ...
    return ensureRoomStatusData().catch(function (e) {
      console.error('[dsh-screeps] roomStatusData seed failed:', e)
    })
  }
  ...
  function installRejectionGuard() {
    if (process.listenerCount('unhandledRejection') === 0) {
      process.on('unhandledRejection', function (reason) {
        console.error('[dsh-screeps] unhandledRejection (process kept alive):')
  ...
  function resetArena() {
    ...
    resetReplayBridge() // M4-A0：清场使旧 replay generation stale（bridge 内 drain/finalize 后丢弃）
    var common = deps()
    ...
    switch (cmd) {
      ...
      case 'resume':
        ...
        return refreshWorldMeta().then(
          ...
          function (e) {
            console.error('[dsh-screeps] world meta refresh on resume failed:', e)
            var writeRooms = requestedRooms
              ? env.set(env.keys.ACCESSIBLE_ROOMS, JSON.stringify(requestedRooms))
              : Promise.resolve()
            return writeRooms.then(function () {
      ...
      case 'resetArena':
        return resetArena()
      case 'replayStart': {
        // M4-A0 canonical replay bridge 启动（正式 schemaVersion=1）
        var rs = value && typeof value === 'object' ? value : {}
        if (typeof rs.replayId !== 'string' || rs.replayId.length === 0) {
          return Promise.reject('replayStart requires {replayId}')
        }
        if (typeof rs.matchId !== 'string' || rs.matchId.length === 0) {
          return Promise.reject('replayStart requires {matchId}')
        }
        var roomsArg = Array.isArray(rs.rooms) ? rs.rooms.filter(function (r) { return typeof r === 'string' }) : []
        if (roomsArg.length === 0) return Promise.reject('replayStart requires non-empty rooms[]')
        var bridge = getReplayBridge()
        var started = bridge.start({
          replayId: rs.replayId,
          matchId: rs.matchId,
          rooms: roomsArg,
          acceptedGameTime: typeof rs.acceptedGameTime === 'number' ? rs.acceptedGameTime : null,
        })
        if (started.error) return Promise.reject(started.error.code === 'conflict' ? 'replay already active' : 'replay start failed')
        return Promise.resolve(started)
      }
      case 'replayPage': {
        var rp = value && typeof value === 'object' ? value : {}
        if (typeof rp.replayId !== 'string') return Promise.reject('replayPage requires {replayId}')
        var bridge2 = getReplayBridge()
        if (!bridge2.state.active || bridge2.state.replayId !== rp.replayId) {
          return Promise.reject('replayPage: unknown or inactive replay')
        }
        if (rp.sourceGeneration && bridge2.state.sourceGeneration !== rp.sourceGeneration) {
          return Promise.reject('replayPage: stale generation')
        }
        var pg = bridge2.page(typeof rp.cursor === 'number' ? rp.cursor : 0, typeof rp.limit === 'number' ? rp.limit : 100)
        if (pg.error) return Promise.reject(pg.error.message || 'replayPage failed')
        return Promise.resolve(pg)
      }
      case 'replayStop': {
        var rstop = value && typeof value === 'object' ? value : {}
        if (typeof rstop.replayId !== 'string') return Promise.reject('replayStop requires {replayId}')
        var bridge3 = getReplayBridge()
        if (!bridge3.state.active || bridge3.state.replayId !== rstop.replayId) {
          return Promise.reject('replayStop: unknown or inactive replay')
        }
        if (rstop.sourceGeneration && bridge3.state.sourceGeneration !== rstop.sourceGeneration) {
          return Promise.reject('replayStop: stale generation')
        }
        return bridge3.stop()
      }
      case 'replayStatus': {
        var bridge4 = getReplayBridge()
        return Promise.resolve(bridge4.status())
      }
      case 'generateRoom': {
      ...
      case 'arenaGen': {
        // A 节：镜像克隆（基准房 + 东邻镜像房），生成双方对称地形/资源/中立 controller。
        // 参数透传照 /rooms 路由（L918-924）：terrainType/sources/mineral/exits 全透传（勿抄
        // system generateRoom 只透传 exits）。基准房 exits.right 开向东邻镜像（B3：镜像=东邻，
        // roomNameFromXY(x+1,y)；示例 W15N15 → W14N15）；水平翻转下 base right ↔ mirror left、
        // y 坐标不变 → 出口格天然对称，exits 登记钉死不写 db.rooms 字段（引擎 interRoom 只读
        // 边界格 terrain，待 IT1 战斗真实跨房验证）。
        ...
        var base = value.room
        var baseXY = common.roomNameToXY(base)
        var mirror = common.getRoomNameFromXY(baseXY[0] + 1, baseXY[1])
        var aGenOpts = {}
        ...
        // controller:true 供后续 pre-assign（plan A 节：保留 controller + placeSpawn 标准赋权）；
        // keeperLairs:false 禁 NPC（禁 Invader/Source Keeper，Arena 短局无骚扰）
        aGenOpts.controller = true
        aGenOpts.keeperLairs = false
        var rightY = value.exits && Array.isArray(value.exits.right) ? value.exits.right : [24, 25]
        aGenOpts.exits = { right: rightY }

        return Promise.resolve()
          // 1) 基准房：stock generateRoom 全链（removeWhere → gen → addWalledNeighbors →
          //    updateTerrainData → addAccessibleRoom）；generateRoom 内部已登记 db.rooms
          .then(function () { return db['rooms.terrain'].removeWhere({ room: base }) })
          .then(function () { return d.cliMap.generateRoom(base, aGenOpts) })
          .then(function (r) { return addWalledNeighbors(base).then(function () { return r }) })
          .then(function (r) { return d.cliMap.updateTerrainData().then(function () { return r }) })
          .then(function (r) { return addAccessibleRoom(base).then(function () { return r }) })
          // 2) 镜像房：清桩 → 反转地形 → 登记 db.rooms → 复制 objects → 邻桩 → 重建 blob → 入 accessible
          .then(function () {
            // 基准房的 addWalledNeighbors 已给镜像房插全墙桩（当时镜像不存在），必须先清
            return db['rooms.terrain'].removeWhere({ room: mirror })
          })
          .then(function () { return db['rooms.terrain'].findOne({ room: base }) })
          .then(function (baseTerrain) {
            if (!baseTerrain) throw 'base terrain missing for ' + base
            return db['rooms.terrain'].insert({ room: mirror, terrain: reverseTerrain(baseTerrain.terrain) })
          })
          .then(function () {
            // 登记（B1/B4：updateTerrainData 以 db.rooms + db['rooms.terrain'] 为准 deflate）
            return db.rooms.insert({ _id: mirror, status: 'normal', sourceKeepers: false })
          })
          .then(function () {
            return db['rooms.objects'].find({ room: base }).then(function (objects) {
              return Promise.all(
                objects
                  ...
                  .map(function (o) {
                    ...
                    if (o.type === 'controller') {
                      // A 节钉死：镜像 controller = 中立副本（user:null、level:0、无状态字段），
                      // 与 source 同路径；房间字段已改为镜像房名。两侧各属一方由 start 的
                      // createUser→placeSpawn 预赋权完成（无中立 controller 可 claim → 禁扩张天然达成）
                      copy.user = null
                      copy.level = 0
                      copy.progress = 0
          ...
          .then(function () { return addWalledNeighbors(mirror) })
          .then(function (r) { return d.cliMap.updateTerrainData().then(function () { return r }) })
          .then(function (r) { return addAccessibleRoom(mirror).then(function () { return r }) })
          .then(function () { return { base: base, mirror: mirror, exits: { right: rightY } } })
      }
      case 'arenaProbe': {
        // M3 A 节契约探针：返回 base/mirror 两房 terrain 编码串 + 对称对象坐标（source/mineral/
        // controller），供 IT 断言「镜像 terrain = 基准逐行反转」「objects 坐标 x'=49-x」。
        // 只读探针，不改变任何状态。
        if (!value || typeof value.base !== 'string' || typeof value.mirror !== 'string') {
          return Promise.reject('arenaProbe requires {base, mirror}')
        }
        var readTerrain = function (room) {
          return db['rooms.terrain'].findOne({ room: room }).then(function (t) {
            return t ? t.terrain : null
          })
        }
        var readObjects = function (room) {
        ...
        return Promise.all([readTerrain(value.base), readTerrain(value.mirror), readObjects(value.base), readObjects(value.mirror)])
          .then(function (r) {
  ...
  module.exports = function arenaMod(config) {
    ...
    if (!config || !config.backend || !config.backend.router) {
      console.log('[dsh-screeps] arena mod: no backend router in this process, routes skipped')
      return
    }
    installRoutes(config)
    console.log('[dsh-screeps] arena mod installed: steam stubbed, /api/arena/* ready')
  }
  ...
  // M4-A0：契约测试直接构造纯 bridge（不依赖 arena-mod db/注入）
  module.exports._createReplayBridge = createReplayBridge
