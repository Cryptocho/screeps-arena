...
async function ensurePortable(
  ...
  const pointerPath = path.join(options.runtimeDir, '.dsh-screeps-resolved.json')
  ...
  const marker = path.join(target, '.dsh-screeps-ok')
  ...
  await writeFile(
    path.join(options.runtimeDir, '.dsh-screeps-resolved.json'),
